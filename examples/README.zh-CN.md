...
| 用例 | 场景 | 机制要点 |
|---|---|---|
| [goal-loop.pcl](goal-loop.pcl) | **目标达成循环**（同其他 agent 的 /goal 功能） | 每轮两 pass（执行+检查），各在全新上下文（`:new`）中运行、prompt 自包含（目标+历史+轮次）；达成即 `:break` |
| [review-prove.pcl](review-prove.pcl) | **审查报告 → 自证循环，直到全部主张可信** | `:while` + `:break`、结构化写回（字符串数组）、跨轮状态（PENDING） |
...
# ② 目标循环（每轮 2 次 LLM 调用，达成即停）
pcl run goal-loop.pcl "在当前目录创建 hello.txt，内容为 Hello PCL" --trace
