...
- 状态：**提案（Proposal，未实现）**——候选进入 v0.2（或 v0.1 打磨轮）；实现前评审定稿
...
3. **零第三方依赖**：格式取 **JSON**（stdlib、与 pi 的 `settings.json` 生态一致）；不支持注释（`"$comment"` 键约定 + 文档兜底）。TOML 因 3.10 无 `tomllib`（引入 `tomli` 违反零依赖）落选。
...
- 环境变量不进入逐键优先级链：仅 `PCL_CONFIG_FILE`（重定向用户级文件）与既有 `PCL_BIN`（连接器 spawn pcl 用，见 §5 嵌入节）。
...
**F. 开发/运维（v1+，可选）**
- `pcl config`（CLI 子命令）：打印**生效配置及各键来源**（builtin/user/project/CLI），只读；`get/set` 写回工具不做（与手工编辑冲突，收益低）。
...
- 未知键：**stderr 警告并继续**（前向兼容：新版加键不炸旧版；拼写错误有提示）。
...
- `pi.args` 拼接 vs 整体替换：拼接已定，但重复项（如两处都给 `--model`）后写胜出属 pi 命令行语义，是否需要在文档强调？
- 连接器（TS）是否纳入同一设置体系（`embed.*`）：涉及 jiti 读文件与两语言路径一致性，建议独立评审。
- `pcl config` 只读命令是否随本方案一并实现。
